...
const ViewportSwatches = ({
  ...
  const sortedItems = useMemo(() => {
    ...
    if (viewportFilter === "pins")
      filtered = filtered.filter((x) => x.type === "pin");
    if (viewportFilter === "anchors")
      filtered = filtered.filter(
        (x) => x.type === "anchor" || x.type === "grid",
      );
                      ...
                      <div className="flex flex-wrap items-center justify-center gap-1 w-full h-full p-0.5">
                        {cellItems.map((item, idx) => (
                          <div
                            ...
                            className={`rounded cursor-pointer hover:ring-2 hover:ring-sky-500 transition-all relative ${activeHex === item.hex ? "ring-4 ring-sky-500 z-20 scale-110" : "z-10"}`}
                            style={{
                ...
                >
                  <td className="p-1 px-3">
                    <div
                      ...
                      style={{
                        backgroundColor: item.hex,
                        backgroundImage: item.note?.startsWith("http")
                          ? `url(${item.note})`
                          : "none",
          ...
          >
            ...
            >
              ...
              <div
                ...
                style={{
                  backgroundColor: item.note?.startsWith("http")
                    ? "rgba(0,0,0,0.3)"
                    : "transparent",
                  color: item.note?.startsWith("http")
                    ? "white"
                    : item.L > 0.65
                      ? "rgba(0,0,0,0.85)"
                      : "rgba(255,255,255,0.95)",
  ...
  const sortedSteps = useMemo(() => {
    ...
    if (searchTerm.trim()) {
      ...
      filtered = filtered.filter((item) =>
        (adjectives[item.lStr] || "").toLowerCase().includes(q),
      );
    }
      ...
      >
        <div className="flex justify-between items-center mb-4 relative z-10">
          ...
          <button
            ...
            className="p-2 hover:bg-slate-100 dark:hover:bg-neutral-800 rounded-full text-slate-500 dark:text-slate-400 pointer-events-auto"
          >
...
const App = () => {
  ...
  const searchResults = useMemo(() => {
    ...
    for (const pt of filteredViewData.points) {
      ...
      const hasDictMatch =
        (adjStr && adjStr.includes(q)) ||
        (nameStr && nameStr.includes(q)) ||
        (fullNameStr && fullNameStr.includes(q)) ||
        (noteStr && noteStr.includes(q)) ||
        (tagsStr && tagsStr.includes(q));
      ...
      for (const [brandKey, list] of Object.entries(colorData)) {
        ...
        for (let listIdx = 0; listIdx < list.length; listIdx++) {
          ...
          const matchesNote =
            ...
            customNote.toLowerCase().includes(q) ||
            itemTags.includes(q) ||
...
const ViewDatabase = ({
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden pt-2 relative bg-slate-50/50 dark:bg-neutral-900/50">
      ...
      {fullscreenImage && (
        <div
          className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 p-8 cursor-pointer"
          onClick={() => setFullscreenImage(null)}
        >
          <img
            src={fullscreenImage}
            ...
            >
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full bg-slate-50 dark:bg-neutral-800 border border-slate-200 dark:border-neutral-700 rounded px-3 py-2 text-xs font-mono h-24 custom-scrollbar"
        ...
        (() => {
          ...
          return (
            <div className="fixed inset-0 z-[100] flex animate-in fade-in duration-300">
              ...
              <div
                className="flex-1 flex flex-col items-center justify-end p-20 relative transition-colors duration-300 cursor-pointer group"
                style={{
                  backgroundColor: hA,
